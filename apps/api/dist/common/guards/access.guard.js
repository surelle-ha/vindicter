...
const require_access_decorator_1 = require("../decorators/require-access.decorator");
let AccessGuard = class AccessGuard {
    constructor(reflector) {
        this.reflector = reflector;
    }
    canActivate(ctx) {
        const required = this.reflector.getAllAndOverride(require_access_decorator_1.ACCESS_KEY, [
            ctx.getHandler(),
            ctx.getClass(),
        ]);
        if (!required)

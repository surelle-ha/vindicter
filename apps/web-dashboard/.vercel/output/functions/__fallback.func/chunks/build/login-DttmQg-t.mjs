...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</button></div></div><div class="flex justify-center pt-1">`);
      ...
      _push(`<button type="submit"${ssrIncludeBooleanAttr(unref(loading)) ? " disabled" : ""} class="w-full rounded-xl py-3 text-[13px] font-bold flex items-center justify-center gap-2 transition-colors bg-accent hover:bg-accent/90 disabled:opacity-60 disabled:cursor-not-allowed text-white">`);
      if (unref(loading)) {
        _push(ssrRenderComponent(unref(Loader2), { class: "h-4 w-4 animate-spin" }, null, _parent));
      } else {
        _push(`<!---->`);
      }
      _push(` ${ssrInterpolate(unref(loading) ? "Signing in…" : "Sign in")}</button></form></div><p class="mt-5 text-center text-[12px]" style="${ssrRenderStyle({ "color": "rgba(255,255,255,0.35)" })}"> New to Vindicter? `);
...
//# sourceMappingURL=login-DttmQg-t.mjs.map
